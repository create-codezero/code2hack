...
if (isset($_POST['down'])) {
     ...
     $q1 = "SELECT * FROM `assests` WHERE `type` = '$type' AND `id` = $id ORDER BY id DESC ";
...
<body>
     ...
     <div id="features" class="bg-dark-blue">
          ...
          <div class="container fit-h justify-center d-flex">
               ...
               <div class="card-container tx-center mb-20 justify-center">
                    ...
                    //selecting all development assests
                    $sql = "SELECT * FROM `assests` WHERE `type` = '$type' ORDER BY id DESC ";
                    ...
                    ?>
                              <div class="card-Team">
                                   ...
                                   <p class="fs-18" class="<?php echo $row['id']; ?>">Total Download: <?php echo $row['download_count']; ?></p>
                                   <p class="iamhere"></p>
                                   <form method="post">
                                        <input type="hidden" name="id" value="<?php echo $row['id']; ?>">
                                        <button type="submit" name="down" class="btn btn-green mb-10" onclick="Sefaaa(this)" id="<?php echo $row['id']; ?>"><a href="<?php echo $row['file']; ?>" download>Download</a></button>
                                   </form>
                                   <!---href="<?php echo $row['file']; ?>" download--->
                              </div>
                    <?php
                         }
                    } else {
     ...
     <script>
          function Sefaaa(unique_id) {
               var asset = unique_id.id;
